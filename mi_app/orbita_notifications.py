...
def notify_orbita_client(client, notification_type, title, message="", link="", request=None):
    """
    ...
    try:
        ...
        if full_url and request:
            dashboard = request.build_absolute_uri(reverse("orbita_dashboard"))
        else:
            dashboard = ""
        ...
        logger.warning("Orbita notify email failed: %s", e)

    return notification


def notify_support_plan_change(user, orbita_client, old_plan_id, new_plan_id):
    """
    Envía un correo a soporte (hola@starpathai.mx) cuando un cliente solicita
    ...
    - orbita_client: cliente o None
    - old_plan_id: str (ej. "FREE")
    - new_plan_id: str (ej. "PRO")
    """
    to_email = getattr(settings, "ORBITA_SUPPORT_EMAIL", "hola@starpathai.mx")
    ...
    company = html.escape(orbita_client.company_name if orbita_client else "—")
    contact = html.escape(orbita_client.contact_name if orbita_client else "—")
    phone_raw = (orbita_client.contact_phone or "").strip() if orbita_client else ""
    ...
    subject = "[ATS] Solicitud de cambio de plan - validar y activar desde admin"
    body_plain = f"""Un cliente ha solicitado cambio de plan en ATS. El plan NO se ha cambiado aún.
...
<body style="margin:0; font-family: 'Segoe UI', Roboto, sans-serif; background:#f0f4f8; padding: 24px;">
  <div style="max-width: 520px; margin: 0 auto; background:#fff; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); overflow: hidden;">
    <div style="background: linear-gradient(135deg, #0B1C2D 0%, #122d47 100%); color: #fff; padding: 20px 24px;">
      ...
      <p style="margin: 6px 0 0; font-size: 0.9rem; opacity: 0.9;">ATS — Validar y activar desde admin</p>
    </div>
    ...
    <div style="padding: 14px 24px; background: #f8fafc; font-size: 0.8rem; color: #666; border-top: 1px solid #eee;">ATS — Star Path</div>
  </div>
</body>
</html>
"""
    try:
        ...
        logger.warning("Orbita notify_support_plan_change failed: %s", e)


def notify_support_account_deletion_request(orbita_client):
    """
    ...
    to_email = to_email.strip()
    company = html.escape(orbita_client.company_name if orbita_client else "—")
    contact = html.escape(orbita_client.contact_name or "—")
    user_email = html.escape(getattr(orbita_client.user, "email", "") or "—")
    subject = "[ATS] Solicitud de baja de cuenta"
    body_plain = f"""Un cliente ha solicitado la baja de su cuenta en ATS.
...
<p>Un cliente ha solicitado la baja de su cuenta en ATS.</p>
...
<p style="margin-top: 20px; color: #666;">Proceder con la baja/eliminación desde el administrador si corresponde.</p>
    ...
    try:
        ...
        logger.warning("Orbita notify_support_account_deletion_request failed: %s", e)
...
def send_email_to_candidate(client, candidate, email_type, custom_message=None):
    ...
    - client: Orbita client (para from_email desde config)
    ...
    from_name = (getattr(config, "company_from_name", "") or "").strip() or client.company_name or "ATS"
    ...
    company_name_html = html.escape(from_name or "ATS")
...
<body style="margin:0; padding:24px; background:#f3f6fb; font-family:'Segoe UI', Roboto, Arial, sans-serif; color:#243447;">
  <div style="max-width:640px; margin:0 auto; background:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 8px 24px rgba(11,28,45,0.08);">
    <div style="background:linear-gradient(135deg, #0b1c2d 0%, #16324f 100%); padding:24px 26px;">
      <p style="margin:0; color:#9fd9ff; font-size:12px; letter-spacing:.08em; text-transform:uppercase;">ATS • Comunicación de reclutamiento</p>
    ...
    <div style="padding:14px 26px; border-top:1px solid #ecf1f7; background:#fafcff;">
      <p style="margin:0; color:#7a8794; font-size:12px;">Este mensaje fue enviado desde ATS. Si tienes dudas, responde a este correo.</p>
    </div>
  </div>
</body>
</html>"""
